...
1. **`create-release`** — creates the GitHub release with notes pulled from `CHANGELOG.md`.
2. **`upload-binaries`** — builds and uploads archives for all six targets
   (Linux x86_64 gnu/musl, aarch64 musl; macOS x86_64/aarch64; Windows x86_64).
3. **`update-tap`** — downloads the four Homebrew-relevant archives
...
three jobs go green. If `update-tap` fails (e.g. an asset 404s because a build
